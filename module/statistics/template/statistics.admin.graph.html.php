...
<?php
	...
	<?php } 
	if( $data['max_total'] < $i2 ) {?>
		<div class='num-label' style='bottom:100%'>
			<div><?php echo $data['max_total'] ?></div>
		</div>
	<?php
	}
		$color_range = [];
		...
		foreach( $data['overall_statistics'] as $collection ){						
			if( !empty( $collection['count'] ) ){
				...
				if( !empty( $collection['border_code'] ) || $collection['border_code'] == 0 ) {
					...
					$inner_inline_style .= ";border-bottom:1px solid;border-color:rgba(".$color_range[0][$border_color].",".$color_range[1][$border_color].",".$color_range[2][$border_color].",1)";
				}
				$inner_inline_style .= ";'";
				
				
				$url = "?list_type=$data[list_type]&date_from=".$collection['date']."&date_to=".$collection['date']."&show_by=".$data['show_by']."&show_all=true";
