...
if ( ! $config = post_config($id) ) {
    $config = post_config()->set('id', $id)->set('name', 'English World')->save();
}

$id_gallery = "englishworld_gallery";
if ( ! $config_gallery = post_config($id_gallery) ) {
    $config_gallery = post_config()->set('id', $id_gallery)->set('name', 'English World Gallery')->save();
}
